...
function mod_schedule_config($id) {
	if (!is_owner($id)) {
		return;
	}
	$menu = array();
...
function mod_schedule_block($id) {
	global $COMUNI_DEBUG, $JQUERY;

	$data = array(id       => $id,
				  editlink => mod_schedule_config($id),
				  title    => 'GLIST',
				  content  => '');
...
$('#schedule_${id}_prev').click(function() {
	...
	$('#schedule_${id}').load('/modules/schedule/reload.php',
...
$('#schedule_${id}_current').click(function() {
	...
	$('#schedule_${id}').load('/modules/schedule/reload.php', { eid:${id}, ajaxmode:1 },
							 function() {
								tb_init('a.thickbox, area.thickbox, input.thickbox');
							 });
});
...
__CAL_HEAD__;
	...
	while ($day = $calMonth->fetch()) {
	    ...
	    if ($day->isEmpty()) { 
			...
			if ($date_href[$df]) {
				$title = $df. 'のスケジュール';
				$link = '/modules/schedule/window.php?pid='. $id. '&date='. $df.
	...
	if (count($sch_week) > 0) {
		...
		foreach ($sch_week as $df => $d) {
			...
			foreach ($d as $a) {
				...
				$link = '/modules/schedule/window.php?eid='. $a["id"].
						'&keepThis=true&TB_iframe=true&height=480&width=640';
