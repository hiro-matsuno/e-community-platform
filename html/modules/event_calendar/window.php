...
if ($pid > 0) {
	$content = mod_event_calendar_bydate($pid);
}
else {
	$content = mod_event_calendar_byid($eid);
}
...
function mod_event_calendar_bydate($blk_id) {
...
<div style="text-align: right;"><a href="/index.php?module=schedule&eid=${id}&blk_id=$blk_id" target="_parent">詳細を見る&raquo;</a></div>
</div>
<hr>
__CONTENT__;
		;
...
function mod_event_calendar_byid($id) {
	global $COMUNI_DEBUG, $JQUERY;
