...
function mod_event_calendar_config($id) {
	if (!is_owner($id)) {
		return;
	}
	$menu = array();
...
function mod_event_calendar_block($id) {
	global $COMUNI_DEBUG, $JQUERY;

	$data = array(id       => $id,
	editlink => mod_event_calendar_config($id),
	title    => 'GLIST',
	content  => '');
	...
	$q = mysql_full("select sd.*, o.uid from schedule_data as sd".
					...
					" and sd.endymd >= now()".
					" and sd.pid = %s".
					" order by sd.startymd limit %s",
	mysql_num(public_status($id)), mysql_num(myuid()),
	mysql_num($id),mysql_num(10));
	if ($q) {
		while ($r = mysql_fetch_array($q)) {
			$att = substr($r["startymd"], 0, 10);
...
$('#schedule_${id}_prev').click(function() {
	...
	$('#schedule_${id}').load('/modules/event_calendar/reload.php',
...
$('#schedule_${id}_current').click(function() {
	...
	$('#schedule_${id}').load('/modules/event_calendar/reload.php', { eid:${id}, ajaxmode:1 },
							 function() {
								tb_init('a.thickbox, area.thickbox, input.thickbox');
							 });
});
...
__CAL_HEAD__;
	...
	while ($day = $calMonth->fetch()) {
		...
		if ($day->isEmpty()) {
			...
			if ($date_href[$df]) {
				$title = $df. 'のスケジュール';
				$link = '/modules/event_calendar/window.php?pid='. $id. '&date='. $df.
	...
	if (count($sch_week) > 0) {
		$buff .= '<div class="schedule_w_t">直近の予定</div>';
		...
		foreach ($sch_week as $df => $d) {
			...
			foreach ($d as $a) {
				...
				$link = '/modules/event_calendar/window.php?eid='. $a["id"].
						'&keepThis=true&TB_iframe=true&height=480&width=640';

				$link = main_href('schedule', $a['id'], $a['blk_id']);

				if(strtotime($a["startymd"]) < time())
					$buff .= '<div class="schedule_now">';
				else
					$buff .= '<div class="schedule_comming">';
				$buff .= '<div class="schedule_date">'. $tm. '</div>';
				$buff .= '<a class="schedule_href" href="'. $link. '"><span>'.
				$a["subject"].'</span></a>';
				if(strtotime($a["startymd"]) < time())
					$buff .= '<span style="color: #FF0000;">開催中！</span>';
				$buff .= '</div>';
			}
		}
	}
...
//function mod_event_calendar_date_count($table) {
