...
function regist_data($eid = null, $pid = null) {
	...
	$target_g = isset($_POST['target_g'])? $_POST['target_g'] : array();
	$target_m = isset($_POST['target_m'])? $_POST['target_m'] : array();
...
//投稿用パーツが外されたときの処理
    mysql_query('lock tables block, reporter_block');
	$d = mysql_exec('update block inner join reporter_block'.
					' on block.id=reporter_block.block_id'.
...
function input_data($eid = null, $pid = null) {
	...
	$SYS_FORM["input"][] = array(title => '投稿ブログパーツ表示メッセージ',
								 name  => 'block_msg',
