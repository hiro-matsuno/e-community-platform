...
function delete_data($eid = null, $pid = null) {
	...
	$d = mysql_exec('delete from map_layer where id = %s',
					mysql_num($id));
...
function list_data($eid = null, $pid = null) {
	...
	$f = mysql_full('select * from map_layer order by id desc');

	if (!$f) {
		show_error('編集するレイヤーがありません。');
	}
	...
	while ($d = mysql_fetch_array($f, MYSQL_ASSOC)) {
		$list[] = array('id'       => make_href('[編集]', 'layer_edit.php?action=edit&id='. $d['id']),
						'del'      => make_href('[削除]', 'layer_edit.php?action=delete&id='. $d['id']),
						'cp_name'  => $d['cp_name'],
...
function edit_data($eid = null, $pid = null) {
	...
	$SYS_FORM["action"] = 'layer_edit.php';
