...
;
	...
	if ($eid > 0) {
		...
		$eid = get_seqid();
		$q = mysql_exec("insert into map_base".
...
function input_data($eid = null, $pid = null) {
	...
	$SYS_FORM["input"][] = array(title => 'マップの名前',
								 name  => 'cp_name',
								 body  => get_form("text", $attr));
	// text:cp_name_short
	$attr = array(name => 'cp_name_short', value => $cp_name_short, size => 16);
	$SYS_FORM["input"][] = array(title => 'マップの名前(省略形)',
								 name  => 'cp_name_short',
								 body  => get_form("text", $attr));
	// text:cp_text
