...
function input_data($eid = null, $pid = null) {
	...
	$SYS_BOX_TITLE = 'ブログ記事一覧';

	$c = mysql_uniq('select * from bosai_web_block where block_id = %s',
					mysql_num($pid));
	if ($c) {
		$SYS_BOX_TITLE = '防災ウェブ投稿一覧';
	}
	$c = mysql_uniq('select * from reporter_block where block_id = %s',
					mysql_num($pid));
	if ($c) {
		$SYS_BOX_TITLE = '市民レポーター投稿一覧';
	}
	...
	$f = mysql_full("select * from blog_data as d".
					" where d.pid = %s".
	...
	// 旧時代の保険
	if (!$f) {
		$f = mysql_full("select * from blog_data".
						" where pid = %s",
						mysql_num($pid));
	}
...
//					body    => '内容',
	...
	if ($f) {
		while ($r = mysql_fetch_array($f)) {
			$href = "/index.php?module=blog&eid=$r[id]&blk_id=${r['blk_id']}";
...
//							updymd  => date('Y年m月d日 H時i分', tm2time($r['updymd'])));
	...
	$edit_url = '/modules/blog/input.php?eid=';
	$del_url  = '/del_content.php?module=blog&eid=';
