...
function entry_data() {
	...
	$d = mysql_exec('delete from core_blacklist_ip_user where uid = %s',
					mysql_num(myuid()));
	...
	foreach ($ip_array as $ip) {
		...
		$i = mysql_exec('insert into core_blacklist_ip_user (uid, ip) values (%s, %s)',
						mysql_num(myuid()), mysql_str($ip));
	}
...
function input_new() {
	...
	$q = mysql_full('select * from core_blacklist_ip_user where uid = %s order by id',
					mysql_num(myuid()));
