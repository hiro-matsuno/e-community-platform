...
$c = mysql_uniq('select * from comment_alt where id = %s', mysql_num($id));
...
if (!isset($_REQUEST["sure"])) {
	$SYS_FORM["action"] = 'comment_del.php';
	$SYS_FORM["submit"] = 'コメント消去';
...
$q = mysql_exec("delete from comment_alt where id = %s", mysql_num($id));

$data = array(title   => 'コメントの削除',
			  icon    => 'finish',
			  content => 'コメントを削除しました。'. reload_form(array(eid => $eid)));
